...
document.addEventListener('DOMContentLoaded', () => {
                ...
                <div class="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 space-y-6">
                    <div class="space-y-3">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="famoso-input" placeholder="Digite o nome do pro player..." class="w-full bg-black/50 border border-zinc-700 rounded-xl py-3 pl-10 pr-4 text-white placeholder:text-zinc-600 focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20 outline-none transition-all">
                        </div>
                        <div class="flex flex-wrap gap-2 pt-2">
                            ${['Nobru', 'Fantasma', 'Two9', 'Marechal', 'Freitas', 'Bak'].map(f => `<button class="chip px-4 py-2 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-500 text-xs font-medium hover:border-orange-500/50 transition-all">${f}</button>`).join('')}
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ${['Balanceada', 'Mais capa', 'Mais controle', 'Muito rápida'].map(s => `<button class="style-btn p-4 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-500 text-sm font-bold hover:border-orange-500/50 transition-all ${s === 'Balanceada' ? 'style-btn-active' : ''}">${s}</button>`).join('')}
                        </div>
                    </div>
                </div>
    ...
    function showResult(nome, estilo, data) {
        ...
        container.innerHTML = `
            <div class="animate-slide-up bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 space-y-6">
                ...
                <div class="bg-black/40 rounded-xl border border-zinc-800/50 overflow-hidden">
                    ${Object.entries(data).map(([label, val]) => `
                        <div class="flex justify-between items-center p-4 border-b border-zinc-800/50 last:border-0">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button class="py-3 rounded-xl bg-zinc-800 text-white font-bold text-sm hover:bg-zinc-700 transition-all">Salvar</button>
                    <button class="py-3 rounded-xl border border-zinc-800 text-zinc-400 font-bold text-sm hover:border-orange-500/50 transition-all">Copiar</button>
                </div>
            </div>
        `;
